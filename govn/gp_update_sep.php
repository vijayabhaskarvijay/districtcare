...
<body>
    ...
    if (!empty($post_id)) {
        $query = "SELECT * FROM govn_posts WHERE gp_id = :post_id";
        ...
        $username = $post['gp_staff_name'];
        $location = $post['gp_loc'];
        $phone = $post['gp_staff_phone'];
        $description = $post['gp_desc'];
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['update'])) {
            ...
            $query = "UPDATE govn_posts SET gp_staff_name = :username,gp_loc = :location, gp_staff_phone = :phone, gp_desc = :description WHERE gp_id = :post_id";
            $stmt = $conn->prepare($query);
            ...
            echo '<script>
                    setTimeout(function() {
                        window.location.href = "govn_manage_post.php";
                    }, 3000);
                </script>';
        }
    }
    ...
    <div class="back-button">
        <a href="govn_manage_post.php" class="go-back">⬅️ GO BACK</a>
    </div>


    <div class="container">
        <h2>Update Govn Post</h2>
